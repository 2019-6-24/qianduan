...
<div id="bar" style="width: 1400px;height:400px;margin-left: 50px;"></div>
...
<script>
    function loadLineColumn() {
        ...
        $.ajax({
            type: 'get',
            data:{tar:tar},
            url: 'http://localhost:8080/cityAreaCharts',//请求数据的地址
...
<script type="text/javascript">
    function loadAreaColumn() {
        ...
        $.ajax({
            type: 'get',
            data:{tar:tar},
            url: 'http://localhost:8080/cityLineCharts',//请求数据的地址
            dataType: "json",        //返回数据形式为json
            success: function (result) {
                //请求成功时执行该函数内容，result即为服务器返回的json对象
                $.each(result.list, function (index, item) {
